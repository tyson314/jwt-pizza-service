...
do
  ...
  token=$(echo $response | jq -r '.token');
  echo "Login diner..."
  curl -s -X POST $host/api/order -H 'Content-Type: application/json' -d '{"franchiseId": 139, "storeId":1, "items":[{ "menuId": 1, "description": "Veggie", "price": 0.05 }]}'  -H "Authorization: Bearer $token" > /dev/null;
  echo "Bought a pizza..."
  sleep 5;
  curl -s -X DELETE $host/api/auth -H "Authorization: Bearer $token" > /dev/null;
