...
orderRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    logger.log('info', 'Factory Request', { diner: { id: req.user.id, name: req.user.name, email: req.user.email }, order });
    ...
    metrics.incrementLatency('factory', factoryLatency);

    const j = await r.json();
    if (r.ok) {
      metrics.calculatePizzaStats(order, 'success');
      res.send({ order, jwt: j.jwt, reportUrl: j.reportUrl });
    } else {
      metrics.calculatePizzaStats(order, 'fail');
      console.error(r);
